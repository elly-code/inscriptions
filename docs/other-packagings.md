...
Please make sure you have these dependencies first before building Inscriptions.
...
To install, use `ninja install`, then execute with `io.github.elly_code.inscriptions`
...
For packagers: A tweak would be to have Inscriptions create a data directory instead of using its root.

Inscriptions just checks whether DATA_DIR exists since in a fresh sandbox it isnt a given, then just dump into it with no regards (since it is expected it does not share the space with other apps) 

Windows has a check in place, you can just remove the "#if WINDOWS"-"#endif" plumbing, and ensure Inscriptions create a folder with rdnn instead of just "Inscriptions" (there is no way to rebase between app-id on windows and other apps dont use rdnn anyway)


## Mac OS?
[An attempt has been made](https://github.com/elly-code/inscriptions/pull/115)
